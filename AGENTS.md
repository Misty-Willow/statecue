...
- Cue taxonomy: `go` / `light` / `rest` / `check`
- Signals: sleep, load, fatigue, data freshness

## Source Of Truth

Before changing product behavior, UI, architecture, or verification rules, read these files in order:
...
6. `specs/001-statecue-mock-direction/spec.md`
7. `specs/001-statecue-mock-direction/plan.md`
8. `specs/001-statecue-mock-direction/tasks.md`
9. `scripts/check.sh`

## Public Repo Boundaries

- Never read or commit `.env`, credential files, private keys, copied auth tokens, or personal health data.
- Use mock/demo data only.
- Do not add secrets, private records, real health data, accounts, auth, wearable integrations, or private integration setup.
- Do not mention unrelated projects, non-public planning context, review context, copied submission text, copied evidence, or internal handoff material.
...
- Do not add `apps/api` unless a future server slice is explicitly selected.
- The first screen must be a usable dashboard, not a marketing landing page.
- The first viewport must show StateCue, the tagline, the Japanese copy, one active cue using `go` / `light` / `rest` / `check`, sleep/load/fatigue/data freshness signals, confidence or clarity, rationale, a mock/demo-data label, and a non-medical safety note.
- Use deterministic mock data. Do not use random defaults, persistence, auth, integration placeholders, or real-looking health fixture files.
- Keep layouts responsive for mobile and desktop.
- Keep UI accessible, restrained, and product-specific. Avoid generic SaaS filler.

## Stack And UI

- Frontend: React + Vite + HeroUI v3 under `apps/web`.
- Backend: Go API under `apps/api` when a server slice is needed.
- Charts/loading: allowed only where they clarify the product experience; HeroUI remains the UI baseline.
- Read local HeroUI docs before using components.
- Do not invent HeroUI APIs.
- Keep CSS import order exactly:
...
- Use HeroUI where it helps, but plain semantic markup is allowed when it produces a stronger result.

## Verification

- Always run `bash scripts/check.sh` before commits.
- If `apps/web/package.json` exists, also run:
